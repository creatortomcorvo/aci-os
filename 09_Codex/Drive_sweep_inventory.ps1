...
$stagingDir = Join-Path $outDir "${safeLabel}_staging"
$inventoryPath = Join-Path $outDir "${safeLabel}_inventory.md"
...
$excludedPathPattern = '(?i)\\System Volume Information\\|\\\$RECYCLE\.BIN\\|\\\.Spotlight-V100\\'
$sensitivePattern = '(?i)\\Praca\b|\\PRACA|\\DELL20|\\MS Pharma\\|\\PRIVATE\b|private|quarterly|kwartal|client|klient|matter|case file|sprawa|real matter|named company|confidential|poufne|board pack|minutes|protoko|report to board|raport zarz|whistleblower report|sygnalist|personal data|dane osobowe|agreement|contract|execution version|umowa|compliance update for|vifor|pfizer|roche|novartis|sandoz|teva|msd|gbo|beigene|polsat|current status|short description of the status|compliance program january|regional compliance|head of compliance|overview on the current status|gmail|fw_|fwd_|invoice|offer|presentation to management'
...
Get-ChildItem -LiteralPath $repoRoot -Recurse -File -ErrorAction SilentlyContinue |
  ForEach-Object { $repoNames[(Normalize-Name $_.BaseName)] = $true }
...
foreach ($file in $allFiles) {
  ...
  if ($repoNames.ContainsKey($norm)) { $dedupeFlags.Add("LIKELY-REPO-COPY") | Out-Null }
...
foreach ($item in $candidates) {
  if ($duplicateHashes.ContainsKey($item.FullName)) {
    if ($item.DedupeFlag -eq "UNIQUE-CANDIDATE") { $item.DedupeFlag = "DUPLICATE-HASH" } else { $item.DedupeFlag += "; DUPLICATE-HASH" }
  }
...
$promising = $candidates |
  ...
  Sort-Object @{Expression={
    $score = 0
    if ($_.FullName -match '(?i)training|szkolen|culture|integrity|ethics|third|due diligence|cms|risk|compliance|korupcj|sanctions|code') { $score += 5 }
...
$lines.Add("# $Label Inventory v0.1") | Out-Null
$lines.Add("") | Out-Null
...
$lines.Add("") | Out-Null
$lines.Add("Root scanned: ``$root``") | Out-Null
$lines.Add("") | Out-Null
$lines.Add("Staging folder: ``99_Parked/$safeLabel`_staging``") | Out-Null
$lines.Add("") | Out-Null
...
$lines.Add("- STAGED means copied to local staging for further review.") | Out-Null
...
$lines.Add("") | Out-Null
$lines.Add("v0.1 - generated $Label compliance inventory and conservative local staging pass.") | Out-Null
